...
function toMonthly(cost) {
  ...
  if (cost.frequency === "one_time") return 0;
  return v;
}
...
function StatusDot({ active }) {
  const color = active ? "#3B6D11" : "#A32D2D";
  return (
      ...
      {data.map((d, i) => (
        <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
          <div style={{ width:"100%", background:color+"22", borderRadius:"3px 3px 0 0", height:height-16, display:"flex", alignItems:"flex-end" }}>
            <div style={{ width:"100%", background:color, borderRadius:"3px 3px 0 0", height:Math.max(2,(d.v/max)*(height-16)) }}/>
          </div>
...
export default function AdminPanel() {
  ...
  async function loadAll() {
    setLoading(true);
    const [{ data:ts }, { data:evs }, { data:pl }, { data:us }] = await Promise.all([
  ...
  const mrr = active.reduce((sum, t) => sum + (PLAN_CFG[t.plan]?.price || 0), 0);
        ...
        <div>
          <div style={{ ...S.card, padding:0, overflow:"hidden" }}>
            {loading ? (
              ...
              <div style={{ padding:40, textAlign:"center", color:"#888", fontSize:13 }}>Sem clientes ainda.</div>
            ) : (
            ...
            <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={() => setEditTenant(null)}>
              <div style={{ background:"#fff", borderRadius:16, padding:28, width:"100%", maxWidth:440 }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display:"grid", gap:12 }}>
                  ...
                  <div style={S.grid2}>
                    <div>
                      <label style={S.label}>Módulo Discover</label>
                    ...
                    <div>
                      <label style={S.label}>Módulo Feedback</label>
                ...
                <div style={{ display:"flex", gap:10, marginTop:20, justifyContent:"flex-end" }}>
                  ...
                  <button style={S.btnPrimary} onClick={async () => {
                    await supabase.from("tenants").update({
                      name:editTenant.name, plan:editTenant.plan, market:editTenant.market,
                      active:editTenant.active, module_discover:editTenant.module_discover,
                      module_feedback:editTenant.module_feedback,
                    }).eq("id", editTenant.id);
        ...
        <div>
          <div style={{ ...S.grid4, marginBottom:20 }}>
            {[
              ...
              { l:"Custo mensal total", v:fmt(totalCosts), sub:costs.length+" itens",    color:"#185FA5" },
            ...
            <div style={S.card}>
                ...
                <div key={cost.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 0", borderBottom:"0.5px solid #f0f0f0" }}>
                  {editCost?.id === cost.id ? (
                    <div style={{ display:"flex", gap:8, flex:1, alignItems:"center", flexWrap:"wrap" }}>
                      <input style={{ ...S.input, width:110 }} value={editCost.label} onChange={e => setEditCost({ ...editCost, label:e.target.value })}/>
                      <input style={{ ...S.input, width:65 }} type="number" value={editCost.amount} onChange={e => setEditCost({ ...editCost, amount:e.target.value })} placeholder="€"/>
                      <select style={{ ...S.input, width:100 }} value={editCost.frequency} onChange={e => setEditCost({ ...editCost, frequency:e.target.value })}>
                    ...
                    <>
                      <div>
                        ...
                        <span style={{ fontSize:11, color:"#aaa", marginLeft:8 }}>
                          {{ monthly:"Mensal", annual:"Anual", variable:"Variável", one_time:"Único" }[cost.frequency] || cost.frequency}
                        </span>
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <span style={{ fontSize:14, fontWeight:500 }}>
                          {cost.frequency==="annual" ? "€"+cost.amount+"/ano (€"+toMonthly(cost).toFixed(0)+"/mês)" : "€"+Number(cost.amount||0).toFixed(0)+"/mês"}
                        </span>
              ...
              {showNewCost && (
                <div style={{ marginTop:12, padding:12, background:"#f9f9f8", borderRadius:8 }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 70px 100px", gap:8, marginBottom:8 }}>
              ...
              <div style={{ marginTop:14, paddingTop:12, borderTop:"0.5px solid #e5e5e5" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  <span style={{ fontSize:13, color:"#888" }}>Total mensal equivalente</span>
                  <span style={{ fontSize:15, fontWeight:500 }}>{fmt(totalCosts)}/mês</span>
                </div>
                {annualCosts > 0 && (
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    <span style={{ fontSize:12, color:"#aaa" }}>Custos anuais a pagar</span>
                    <span style={{ fontSize:12, color:"#aaa" }}>€{annualCosts}/ano</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {beSimulation.map(row => {
                    ...
                    const current = tenants.filter(t => t.plan !== "trial").length === row.clients;
                    return (
                ...
                <div key={i} style={{ marginBottom:16 }}>
                  ...
                  {i < funnel.length-1 && funnel[i+1].v > 0 && step.v > 0 && (
                    <p style={{ fontSize:10, color:"#ddd", margin:"4px 0 0 28px" }}>
                      {Math.round((funnel[i+1].v / step.v) * 100)}% avançaram para próxima etapa
                    </p>
                  )}
                </div>
              );
        ...
        <div>
          <div style={{ ...S.card, marginBottom:20 }}>
            ...
            <p style={{ fontSize:12, color:"#aaa", marginBottom:16 }}>
              Margem de segurança define quantas pesquisas o cliente pode fazer. Ex: 500 × 60% = 300 permitidas.
            </p>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
              <thead>
                <tr>{["Plano","Preço/mês","Pesquisas/mês","Margem %","Permitidas","Custo Google","Custo IA","Margem €",""].map(h => <th key={h} style={S.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {(planLimits.length ? planLimits : Object.entries(PLAN_CFG).map(([k,v]) => ({
                  plan:k, price_eur:v.price,
                  searches_month:({ trial:20, starter:200, pro:500, enterprise:2000 })[k],
                  margin_pct:0.6,
                }))).map((pl, i) => {
                  ...
                  const marginEur = (pl.price_eur - totalCostPl - totalCosts).toFixed(0);
                  ...
                  return (
                    <tr key={pl.plan} style={{ background:i%2===0?"transparent":"#fafaf9" }}>
                      ...
                      <td style={S.td}>
                        {isEditing
                          ? <input style={{ ...S.input, width:70 }} type="number" value={editPlan.searches_month} onChange={e => setEditPlan({ ...editPlan, searches_month:Number(e.target.value) })}/>
                          : pl.searches_month}
                      </td>
                      <td style={S.td}>
                        {isEditing
                          ? <input style={{ ...S.input, width:60 }} type="number" step="0.05" min="0.1" max="1" value={editPlan.margin_pct} onChange={e => setEditPlan({ ...editPlan, margin_pct:Number(e.target.value) })}/>
                          : Math.round((pl.margin_pct || 0.6) * 100) + "%"}
                      </td>
                          ...
                          <div style={{ display:"flex", gap:6 }}>
                            <button style={S.btnPrimary} onClick={async () => {
                              await supabase.from("plan_limits").upsert({
                                plan:editPlan.plan, searches_month:editPlan.searches_month,
                                margin_pct:editPlan.margin_pct, price_eur:editPlan.price_eur,
                              }, { onConflict:"plan" });
          ...
          <div style={S.card}>
            ...
            {usageSummaries.length === 0 ? (
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                <thead>
                  <tr>{["Cliente","Plano","Pesquisas","Permitidas","Uso %","Custo IA ($)","Estado","Reset"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr>
                </thead>
                <tbody>
